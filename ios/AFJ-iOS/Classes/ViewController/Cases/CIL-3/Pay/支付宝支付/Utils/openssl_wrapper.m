...
int rsa_sign_with_private_key_pem(char *message, int message_length, unsigned char *signature, unsigned int *signature_length, char *private_key_file_path, BOOL rsa2) {
    ...
    if (rsa_private != nil) {
        if (1 == RSA_check_key(rsa_private)) {
            int rsa_sign_valid = RSA_sign((rsa2 ? (NID_sha256) : (NID_sha1)), shabuf, (rsa2 ? (SHA256_DIGEST_LENGTH) : (SHA_DIGEST_LENGTH)), signature, signature_length, rsa_private);
            if (1 == rsa_sign_valid) {
                success = 1;
            }
        }
