...
@interface AFJPDFSampleViewController () <JFThumbnailViewControllerDelegate, JFOutlineViewControllerDelegate, JFSearchViewControllerDelegate>
...
@implementation AFJPDFSampleViewController

- (instancetype)initWithPdfName:(NSString *)name {
    self = [super init];
    if (self) {
        _pdfName = name;
    }
...
- (void)viewDidLoad {
    ...
    [self setupPDFView:_pdfName];
}

- (void)setupPDFView:(NSString *)pdfName {
    self.title = pdfName;
