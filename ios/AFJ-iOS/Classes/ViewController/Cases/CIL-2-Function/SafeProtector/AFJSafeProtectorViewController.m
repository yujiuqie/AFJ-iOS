...
- (void)viewDidLoad {
    ...
    NSMutableArray *dataArray = [NSMutableArray array];
    {
        ...
        item.cName = @"testUnrecognizedSelector";
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"testMutableDictionary";
        item.didSelectAction = ^(id data) {
            if(weakSelf.menuItem.enabled){
                [self showAlertWithTitle:@"提示" message:@"尚未开启防护，点击”确定“会直接崩溃" complete:^(NSString * _Nonnull info) {
                    if([info isEqual:@"确定"]){
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"testAttributedString";
        item.didSelectAction = ^(id data) {
            if(weakSelf.menuItem.enabled){
                [self showAlertWithTitle:@"提示" message:@"尚未开启防护，点击”确定“会直接崩溃" complete:^(NSString * _Nonnull info) {
                    if([info isEqual:@"确定"]){
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"testMutableAttributedString";
        item.didSelectAction = ^(id data) {
            if(weakSelf.menuItem.enabled){
                [self showAlertWithTitle:@"提示" message:@"尚未开启防护，点击”确定“会直接崩溃" complete:^(NSString * _Nonnull info) {
                    if([info isEqual:@"确定"]){
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"testNSMutableOrderSet";
        item.didSelectAction = ^(id data) {
            if(weakSelf.menuItem.enabled){
                [self showAlertWithTitle:@"提示" message:@"尚未开启防护，点击”确定“会直接崩溃" complete:^(NSString * _Nonnull info) {
                    if([info isEqual:@"确定"]){
