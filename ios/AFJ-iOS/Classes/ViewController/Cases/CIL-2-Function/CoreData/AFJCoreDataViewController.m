//
//  AFJCoreDataViewController.m
//  AFJ-iOS
//
//  Created by Alfred on 2022/9/24.
//

#import "AFJCoreDataViewController.h"
...
@interface AFJCoreDataViewController()

@property (nonatomic, strong) GPJDataDrivenTableView  *tableView;

@end

@implementation AFJCoreDataViewController
...
- (void)viewDidLoad {
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"makeWorkers";
        item.didSelectAction = ^(id data) {
            [weakSelf makeWorkers];
        };
        [dataArray addObject:item];
    }
    {
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"makeStudentsAndTeachers";
        item.didSelectAction = ^(id data) {
            [weakSelf makeStudentsAndTeachers];
        };
        [dataArray addObject:item];
    }
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"modifyWorker";
        item.didSelectAction = ^(id data) {
            [weakSelf modifyWorker];
        };
        [dataArray addObject:item];
    }
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"makeSnackOnOtherThread";
        item.didSelectAction = ^(id data) {
            [weakSelf makeSnackOnOtherThread];
        };
        [dataArray addObject:item];
    }
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"makeSnackOnOtherThread2";
        item.didSelectAction = ^(id data) {
            [weakSelf makeSnackOnOtherThread2];
        };
        [dataArray addObject:item];
    }
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"delete All Snack";
        item.didSelectAction = ^(id data) {
            [Snack deleteAll];
        };
        [dataArray addObject:item];
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"delete All CDStudent and Teacher";
        item.didSelectAction = ^(id data) {
            [CDStudent deleteAllWithCompletionHandler:^{
                [Teacher deleteAll];
            }];
        };
        [dataArray addObject:item];
    ...
    {
        AFJCaseItemData *item = [AFJCaseItemData new];
        item.cName = @"delete All Team, Coach and Player";
        item.didSelectAction = ^(id data) {
...
- (void)makeStudentsAndTeachers {
    ...
    for (int i = 1; i < 8; i++) {
        ...
        CDStudent *student = [CDStudent instanceWithId:i];
        student.teachers = teachers;
        [students addObject:student];
    }
    [CDStudent saveObjects:students];
...
//    [CDStudent findAllWithCompletionHandler:^(NSArray *objects) {
//        [objects enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//            NSLog(@"******************%s-[CDStudent]******************", __func__);
//            NSLog(@"%p", obj);
//            [obj log];
//            NSLog(@"******************%s-[CDStudent]******************", __func__);
//        }];
