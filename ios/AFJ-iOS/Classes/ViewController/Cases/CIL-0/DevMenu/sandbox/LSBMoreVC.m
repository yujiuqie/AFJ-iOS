...
@interface LSBProgress : UIView
@property(nonatomic, assign) float progress;
@property(nonatomic, weak) UIView *blackView;
@end

@implementation LSBProgress
...
@interface LSBMoreVC () <UITableViewDelegate, UITableViewDataSource, NSURLSessionDelegate>
...
// 下载相关
@property(nonatomic, weak) LSBProgress *progress;
@end

@implementation LSBMoreVC {
    LSBFileModel *_longPressFile; // 记录长按的文件
...
- (void)viewDidLoad {
    ...
    self.fileManager = [LSBFileManager shareManager];

    if (self.fileModel == nil) { // 总目录
        self.homePath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"LSBFileCache"];
        // 创建一个目录
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (file.fileType == LSBFileTypeDirectory) { // 文件夹
        ...
        cell.imageView.image = [LSBHelper imageWithName:@"file_floder"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (file.fileType == LSBFileTypeDirectory) {
        LSBMoreVC *vc = [[LSBMoreVC alloc] init];
        vc.fileModel = file;
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    ...
    _totolLen = (NSUInteger) response.expectedContentLength;
...
- (void)initViews {
    ...
    LSBProgress *progress = [[LSBProgress alloc] initWithFrame:CGRectMake(10, self.view.bounds.size.height - 30, self.view.bounds.size.width - 20, 20)];
    progress.progress = 0;
...
- (void)addAction {
    ...
    UIAlertAction *searchAction = [UIAlertAction actionWithTitle:@"搜索“的”字文件" style:UIAlertActionStyleDefault handler:^(UIAlertAction *_Nonnull action) {
        ...
        for (LSBFileModel *file in resFiles) {
            NSLog(@"%@", file.name);
        }
...
// 复制
- (void)copyFile {
    LSBFileModel *directoryFile;
    for (LSBFileModel *file in self.files) {  // 获取一个临时的最前的文件夹
        if (file.fileType == LSBFileTypeDirectory) {
            directoryFile = file;
        }
    }
