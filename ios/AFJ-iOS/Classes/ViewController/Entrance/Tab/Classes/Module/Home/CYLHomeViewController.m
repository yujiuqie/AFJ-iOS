...
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"refresh TabBar" style:UIBarButtonItemStylePlain target:self action:@selector(refreshTabBar:)];
    ...
    self.itemList = [[AFJProductManager sharedInstance] fullItemList];
}
...
- (void)configureCell:(UITableViewCell *)cell forIndexPath:(NSIndexPath *)indexPath {
    AFJCaseItemData *item = self.itemList[indexPath.row];
    [[cell textLabel] setText:item.cName];
}
...
//    [self cyl_showBadgeValue:[NSString stringWithFormat:@"%@", @(indexPath.row)] animationType:CYLBadgeAnimationTypeScale];
    [self pushToNewViewController:indexPath];
}

- (void)pushToNewViewController:(NSIndexPath *)indexPath {
    AFJCaseItemData *item = self.itemList[indexPath.row];
    AFJSearchListViewController *searchVC = [[AFJSearchListViewController alloc] initWith:item];
